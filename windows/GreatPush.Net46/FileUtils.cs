...
namespace GreatPush.ReactNative
{
    internal class FileUtils
    {
        ...
        internal async static Task ClearReactDevBundleCacheAsync()
        {
            ...
            if (await FileSystem.Current.LocalStorage.CheckExistsAsync(GreatPushConstants.ReactDevBundleCacheFileName).ConfigureAwait(false) != ExistenceCheckResult.FileExists)
                return;

            var devBundleCacheFile = await FileSystem.Current.LocalStorage.GetFileAsync(GreatPushConstants.ReactDevBundleCacheFileName).ConfigureAwait(false);
