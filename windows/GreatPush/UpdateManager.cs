...
namespace GreatPush.ReactNative
{
    internal class UpdateManager
    {
        ...
        {
            await (await GetGreatPushFolderAsync().ConfigureAwait(false)).DeleteAsync().AsTask().ConfigureAwait(false);
        }
        ...
        internal async Task DownloadPackageAsync(JObject updatePackage, string expectedBundleFileName, Progress<HttpProgress> downloadProgress)
        {
            ...
            StorageFolder greatPushFolder = await GetGreatPushFolderAsync().ConfigureAwait(false);
            ...
            StorageFile newUpdateMetadataFile = await newUpdateFolder.CreateFileAsync(GreatPushConstants.PackageFileName).AsTask().ConfigureAwait(false);
            ...
            try
            {
                ...
                StorageFile diffManifestFile = (StorageFile)await unzippedFolder.TryGetItemAsync(GreatPushConstants.DiffManifestFileName).AsTask().ConfigureAwait(false);
        ...
        internal async Task<string> GetCurrentPackageHashAsync()
        {
            ...
            string currentPackageShortHash = (string)info[GreatPushConstants.CurrentPackageKey];
        ...
        internal async Task<StorageFolder> GetPackageFolderAsync(string packageHash, bool createIfNotExists)
        {
            ...
            try
            {
                ...
                return createIfNotExists
                    ? await greatPushFolder.CreateFolderAsync(packageHash, CreationCollisionOption.OpenIfExists).AsTask().ConfigureAwait(false)
                    : await greatPushFolder.GetFolderAsync(packageHash).AsTask().ConfigureAwait(false);
            }
        ...
        {
            JObject info = await GetCurrentPackageInfoAsync().ConfigureAwait(false);
            string previousPackageShortHash = (string)info[GreatPushConstants.PreviousPackageKey];
        ...
        private async Task<StorageFolder> GetGreatPushFolderAsync()
        {
            return await ApplicationData.Current.LocalFolder.CreateFolderAsync(GreatPushConstants.GreatPushFolderPrefix, CreationCollisionOption.OpenIfExists).AsTask().ConfigureAwait(false);
        }
        ...
        {
            var greatPushFolder = await GetGreatPushFolderAsync().ConfigureAwait(false);
            return await greatPushFolder.CreateFileAsync(GreatPushConstants.DownloadFileName, CreationCollisionOption.OpenIfExists).AsTask().ConfigureAwait(false);
        }

        private async Task<StorageFile> GetStatusFileAsync()
        {
            StorageFolder greatPushFolder = await GetGreatPushFolderAsync().ConfigureAwait(false);
            return await greatPushFolder.CreateFileAsync(GreatPushConstants.StatusFileName, CreationCollisionOption.OpenIfExists).AsTask().ConfigureAwait(false);
        }

        private async Task<StorageFolder> CreateUnzippedFolderAsync()
        {
            StorageFolder greatPushFolder = await GetGreatPushFolderAsync().ConfigureAwait(false);
            var unzippedFolder = await greatPushFolder.TryGetItemAsync(GreatPushConstants.UnzippedFolderName).AsTask().ConfigureAwait(false);
            ...
            return await greatPushFolder.CreateFolderAsync(GreatPushConstants.UnzippedFolderName, CreationCollisionOption.OpenIfExists).AsTask().ConfigureAwait(false);
        }
