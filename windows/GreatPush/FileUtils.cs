...
namespace GreatPush.ReactNative
{
    internal class FileUtils
    {
        ...
        {
            var devBundleCacheFile = (StorageFile)await ApplicationData.Current.LocalFolder.TryGetItemAsync(GreatPushConstants.ReactDevBundleCacheFileName).AsTask().ConfigureAwait(false);
        ...
        {
            var assetJSBundleFile = await StorageFile.GetFileFromApplicationUriAsync(new Uri(GreatPushConstants.AssetsBundlePrefix + fileName)).AsTask().ConfigureAwait(false);
