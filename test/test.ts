...
/// <reference path="../node_modules/great-push-plugin-testing-framework/typings/great-push-plugin-testing-framework.d.ts" />
...
import { Platform, PluginTestingFramework, ProjectManager, setupTestRunScenario, setupUpdateScenario, ServerUtil, TestBuilder, TestConfig, TestUtil } from "great-push-plugin-testing-framework";
...
class RNAndroid extends Platform.Android implements RNPlatform {
    ...
    installPlatform(projectDirectory: string): Q.Promise<void> {
        ...
        //// Set up gradle to build GreatPush with the app
        // Add GreatPush to android/app/build.gradle
        var buildGradle = path.join(innerprojectDirectory, "android", "app", "build.gradle");
        TestUtil.replaceString(buildGradle,
            "apply from: \"../../node_modules/react-native/react.gradle\"",
            "apply from: \"../../node_modules/react-native/react.gradle\"\napply from: \"" + path.join(innerprojectDirectory, "node_modules", "react-native-great-push", "android", "greatpush.gradle") + "\"");
        TestUtil.replaceString(buildGradle,
            "compile \"com.facebook.react:react-native:+\"",
            "compile \"com.facebook.react:react-native:0.25.+\"");
        TestUtil.replaceString(buildGradle,
            "// From node_modules",
            "\n    compile project(':react-native-great-push') // From node_modules");
        // Add GreatPush to android/settings.gradle
        ...
        var mainActivity = path.join(innerprojectDirectory, "android", "app", "src", "main", "java", "com", "microsoft", "greatpush", "test", "MainActivity.java");
        TestUtil.replaceString(mainActivity, TestUtil.GREAT_PUSH_TEST_APP_NAME_PLACEHOLDER, TestConfig.TestAppName);
...
class RNIOS extends Platform.IOS implements RNPlatform {
    ...
    installPlatform(projectDirectory: string): Q.Promise<void> {
        ...
        return TestUtil.getProcessOutput("pod init", { cwd: iOSProject })
            ...
            .then<void>(TestUtil.replaceString.bind(undefined, appDelegatePath, TestUtil.GREAT_PUSH_TEST_APP_NAME_PLACEHOLDER, TestConfig.TestAppName));
    }
...
class RNProjectManager extends ProjectManager {
    ...
    public setupScenario(projectDirectory: string, appId: string, templatePath: string, jsPath: string, targetPlatform: Platform.IPlatform, version?: string): Q.Promise<void> {
        ...
        return TestUtil.copyFile(templateIndexPath, destinationIndexPath, true)
            .then<void>(TestUtil.replaceString.bind(undefined, destinationIndexPath, TestUtil.GREAT_PUSH_TEST_APP_NAME_PLACEHOLDER, TestConfig.TestAppName))
            ...
            .then<void>(TestUtil.replaceString.bind(undefined, destinationIndexPath, TestUtil.GREAT_PUSH_APP_VERSION_PLACEHOLDER, version));
    }

    /**
     * Creates a GreatPush update package zip for a project.
     */
    public createUpdateArchive(projectDirectory: string, targetPlatform: Platform.IPlatform, isDiff?: boolean): Q.Promise<string> {
        var bundleFolder: string = path.join(projectDirectory, TestConfig.TestAppName, "GreatPush/");
...
PluginTestingFramework.initializeTests(new RNProjectManager(), supportedTargetPlatforms,
    (projectManager: ProjectManager, targetPlatform: Platform.IPlatform) => {
        TestBuilder.describe("#window.greatPush.checkForUpdate",
            () => {
                TestBuilder.it("window.greatPush.checkForUpdate.noUpdate", false,
                    (done: MochaDone) => {
                ...
                TestBuilder.it("window.greatPush.checkForUpdate.sendsBinaryHash", false,
                    (done: MochaDone) => {
                ...
                TestBuilder.it("window.greatPush.checkForUpdate.noUpdate.updateAppVersion", false,
                    (done: MochaDone) => {
                ...
                TestBuilder.it("window.greatPush.checkForUpdate.update", true,
                    (done: MochaDone) => {
                ...
                TestBuilder.it("window.greatPush.checkForUpdate.error", false,
                    (done: MochaDone) => {
        ...
        TestBuilder.describe("#window.greatPush.checkForUpdate.customKey",
            () => {
                TestBuilder.it("window.greatPush.checkForUpdate.customKey.update", false,
                (done: MochaDone) => {
                    var updateResponse = ServerUtil.createUpdateResponse();
                    ServerUtil.updateResponse = { updateInfo: updateResponse };
        ...
        TestBuilder.describe("#greatPush.restartApplication",
            () => {
                TestBuilder.it("greatPush.restartApplication.checkPackages", true,
                    (done: MochaDone) => {
                        ServerUtil.updateResponse = { updateInfo: ServerUtil.createUpdateResponse(false, targetPlatform) };
        ...
        TestBuilder.describe("#greatPush.restartApplication.2x",
            () => {
        ...
        TestBuilder.describe("#window.greatPush.sync",
            () => {
                ...
                TestBuilder.describe("#window.greatPush.sync 1x",
                    () => {
                        // Tests where sync is called just once
                        TestBuilder.it("window.greatPush.sync.noupdate", false,
                            (done: MochaDone) => {
                                var noUpdateResponse = ServerUtil.createDefaultResponse();
                                noUpdateResponse.isAvailable = false;
                                noUpdateResponse.appVersion = "0.0.1";
                        ...
                        TestBuilder.it("window.greatPush.sync.checkerror", false,
                            (done: MochaDone) => {
                                ServerUtil.updateResponse = "invalid {{ json";
                        ...
                        TestBuilder.it("window.greatPush.sync.downloaderror", false,
                            (done: MochaDone) => {
                        ...
                        TestBuilder.it("window.greatPush.sync.dorevert", false,
                            (done: MochaDone) => {
                                ServerUtil.updateResponse = { updateInfo: ServerUtil.createUpdateResponse(false, targetPlatform) };
                        ...
                        TestBuilder.it("window.greatPush.sync.update", false,
                            (done: MochaDone) => {
                                ServerUtil.updateResponse = { updateInfo: ServerUtil.createUpdateResponse(false, targetPlatform) };
                ...
                TestBuilder.describe("#window.greatPush.sync 2x",
                    () => {
                        // Tests where sync is called again before the first sync finishes
                        TestBuilder.it("window.greatPush.sync.2x.noupdate", false,
                            (done: MochaDone) => {
                                var noUpdateResponse = ServerUtil.createDefaultResponse();
                                noUpdateResponse.isAvailable = false;
                                noUpdateResponse.appVersion = "0.0.1";
                        ...
                        TestBuilder.it("window.greatPush.sync.2x.checkerror", false,
                            (done: MochaDone) => {
                                ServerUtil.updateResponse = "invalid {{ json";
                        ...
                        TestBuilder.it("window.greatPush.sync.2x.downloaderror", false,
                            (done: MochaDone) => {
                                var invalidUrlResponse = ServerUtil.createUpdateResponse();
                                invalidUrlResponse.downloadURL = path.join(TestConfig.templatePath, "invalid_path.zip");
                                ServerUtil.updateResponse = { updateInfo: invalidUrlResponse };
                        ...
                        TestBuilder.it("window.greatPush.sync.2x.dorevert", false,
                            (done: MochaDone) => {
                                ServerUtil.updateResponse = { updateInfo: ServerUtil.createUpdateResponse(false, targetPlatform) };
                        ...
                        TestBuilder.it("window.greatPush.sync.2x.update", true,
                            (done: MochaDone) => {
                                ServerUtil.updateResponse = { updateInfo: ServerUtil.createUpdateResponse(false, targetPlatform) };
        ...
        TestBuilder.describe("#window.greatPush.sync minimum background duration tests",
            () => {
        ...
        TestBuilder.describe("#window.greatPush.sync mandatory install mode tests",
            () => {
