...
// This module wraps GreatPush API calls to add test message callbacks to every function for simpler test code.

module.exports = {
    checkForUpdate: function(testApp, onSuccess, onError, deploymentKey) {
        return GreatPush.checkForUpdate(deploymentKey)
            .then((remotePackage) => {
    ...
    install: function(testApp, onSuccess, onError, installMode, minBackgroundDuration, localPackage) {
        return localPackage.install(installMode, minBackgroundDuration)
            .then(() => {
                ...
                if (installMode !== GreatPush.InstallMode.IMMEDIATE) return testApp.installSuccess().then(() => { return onSuccess && onSuccess(); });
    ...
    sync: function(testApp, onSyncStatus, onSyncError, options) {
        return GreatPush.checkForUpdate()
            .then(
                (remotePackage) => {
                    ...
                    var regularUpdateIsImmediate = options && options.installMode === GreatPush.InstallMode.IMMEDIATE;
                    var mandatoryUpdateIsImmediate = !options || (options && (!options.mandatoryInstallMode || options.mandatoryInstallMode === GreatPush.InstallMode.IMMEDIATE));
                    var isInstallImmediate = (remotePackage && remotePackage.isMandatory) ? mandatoryUpdateIsImmediate : regularUpdateIsImmediate;
                    
                    return GreatPush.sync(options)
                        .then((status) => {
                            if (!(isInstallImmediate && status === GreatPush.SyncStatus.UPDATE_INSTALLED)) {
