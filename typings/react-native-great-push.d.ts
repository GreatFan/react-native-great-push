...
export type SyncStatusChangedCallback = (status: GreatPush.SyncStatus) => void;
export type HandleBinaryVersionMismatchCallback = (update: RemotePackage) => void;

export interface GreatPushOptions extends SyncOptions {
    /**
     * Specifies when you would like to synchronize updates with the GreatPush server.
     * Defaults to greatPush.CheckFrequency.ON_APP_START.
     */
    checkFrequency: GreatPush.CheckFrequency;
}
...
export interface LocalPackage extends Package {
     ...
     */
    install(installMode: GreatPush.InstallMode, minimumBackgroundDuration?: number): Promise<void>;
}
...
export interface Package {
    ...
    /**
     * The internal label automatically given to the update by the GreatPush server. This value uniquely identifies the update within its deployment.
     */
    label: string;
...
export interface RemotePackage extends Package {
    /**
     * Downloads the available update from the GreatPush service.
     *
...
export interface SyncOptions {
     ...
     * Defaults to greatPush.InstallMode.ON_NEXT_RESTART.
     */
    installMode?: GreatPush.InstallMode;

    /**
     * Specifies when you would like to install updates which are marked as mandatory.
     * Defaults to greatPush.InstallMode.IMMEDIATE.
     */
    mandatoryInstallMode?: GreatPush.InstallMode;
...
export interface StatusReport {
     ...
     */
    status: GreatPush.DeploymentStatus;
...
/**
 * Decorates a React Component configuring it to sync for updates with the GreatPush server.
 *
 ...
 */
declare function GreatPush(options?: GreatPushOptions): (x: any) => any;

declare namespace GreatPush {
    /**
    ...
    /**
     * Asks the GreatPush service whether the configured app deployment has an update available.
     *
     * @param deploymentKey The deployment key to use to query the GreatPush server for an update.
     * 
    ...
    /**
     * Notifies the GreatPush runtime that an installed update is considered successful.
     */
    function notifyAppReady(): Promise<StatusReport|void>;

    /**
     * Allow GreatPush to restart the app.
     */
    function allowRestart(): void;

    /**
     * Forbid GreatPush to restart the app.
     */
    function disallowRestart(): void;

    /**
     * Clear all downloaded GreatPush updates.
    ...
    enum SyncStatus {
        /**
         * The app is up-to-date with the GreatPush server.
         */
        UP_TO_DATE,
        ...
        /**
         * The GreatPush server is being queried for an update.
         */
        CHECKING_FOR_UPDATE,
        ...
        /**
         * An available update is being downloaded from the GreatPush server.
         */
        DOWNLOADING_PACKAGE,
    ...
    /**
     * Indicates when you would like to check for (and install) updates from the GreatPush server.
     */
    enum CheckFrequency {
        /**
        ...
        /**
         * Don't automatically check for updates, but only do it when greatPush.sync() is manully called inside app code.
         */
        MANUAL
    }
}

export default GreatPush;
