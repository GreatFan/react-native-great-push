...
@interface GreatPush () <RCTBridgeModule, RCTFrameUpdateObserver>
@end

@implementation GreatPush {
    BOOL _hasResumeListener;
    BOOL _isFirstRunAfterUpdate;
    int _minimumBackgroundDuration;
    NSDate *_lastResignedDate;
    GreatPushInstallMode _installMode;
...
static NSString *const DownloadProgressEvent = @"GreatPushDownloadProgress";
...
static NSString *const FailedUpdatesKey = @"GREAT_PUSH_FAILED_UPDATES";
static NSString *const PendingUpdateKey = @"GREAT_PUSH_PENDING_UPDATE";
...
+ (NSString *)bundleAssetsPath
{
    ...
    return [resourcePath stringByAppendingPathComponent:[GreatPushUpdateUtils assetsFolderName]];
}
                         ...
                         bundle:(NSBundle *)resourceBundle
{
    ...
    NSError *error;
    NSString *packageFile = [GreatPushPackage getCurrentPackageBundlePath:&error];
    ...
    NSString *binaryAppVersion = [[GreatPushConfig current] appVersion];
    NSDictionary *currentPackageMetadata = [GreatPushPackage getCurrentPackage:&error];
    ...
    if ([[GreatPushUpdateUtils modifiedDateStringOfFileAtURL:binaryBundleURL] isEqualToString:packageDate] && ([GreatPush isUsingTestConfiguration] ||[binaryAppVersion isEqualToString:packageAppVersion])) {
...
{
    [GreatPushConfig current].appVersion = appVersion;
}

+ (void)setDeploymentKey:(NSString *)deploymentKey
{
    [GreatPushConfig current].deploymentKey = deploymentKey;
}
...
/*
 * This returns a boolean value indicating whether GreatPush has
...
{
    // Export the values of the GreatPushInstallMode and GreatPushUpdateState
    // enums so that the script-side can easily stay in sync
    return @{
             @"greatPushInstallModeOnNextRestart":@(GreatPushInstallModeOnNextRestart),
             @"greatPushInstallModeImmediate": @(GreatPushInstallModeImmediate),
             ...
             @"greatPushUpdateStateRunning": @(GreatPushUpdateStateRunning),
...
+ (void)ensureBinaryBundleExists
{
    if (![self binaryBundleURL]) {
        NSString *errorMessage;
            ...
            "If you'd like to test GreatPush using the simulator, you can do one of the following depending on your "
            "React Native version and/or preferred workflow:\n\n"

            "1. Update your AppDelegate.m file to load the JS bundle from the packager instead of from GreatPush. "
            "You can still test your GreatPush update experience using this workflow (Debug builds only).\n\n"
        ...
        RCTFatal([GreatPushErrorUtils errorWithMessage:errorMessage]);
    }
}
 ...
 * to point at the latest GreatPush update, and then restarts
...
- (void)loadBundle
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if ([GreatPush isUsingTestConfiguration] || ![super.bridge.bundleURL.scheme hasPrefix:@"http"]) {
            [super.bridge setValue:[GreatPush bundleURL] forKey:@"bundleURL"];
        }
...
{
    NSError *error;
    NSDictionary *failedPackage = [GreatPushPackage getCurrentPackage:&error];
    if (!failedPackage) {
        if (error) {
    ...
    [GreatPushPackage rollbackPackage];
    [GreatPush removePendingUpdate];
    [self loadBundle];
}
...
- (void)applicationWillResignActive
{
    ...
    if (_installMode == GreatPushInstallModeOnNextSuspend && [[self class] isPendingUpdate:nil]) {
                        ...
                        rejecter:(RCTPromiseRejectBlock)reject)
{
    ...
    NSURL *binaryBundleURL = [GreatPush binaryBundleURL];
    if (binaryBundleURL != nil) {
        [mutableUpdatePackage setValue:[GreatPushUpdateUtils modifiedDateStringOfFileAtURL:binaryBundleURL]
    ...
    NSString * publicKey = [[GreatPushConfig current] publicKey];
    ...
    [GreatPushPackage
        ...
        doneCallback:^{
            NSError *err;
            NSDictionary *newPackage = [GreatPushPackage getPackage:mutableUpdatePackage[PackageHashKey] error:&err];
        ...
        failCallback:^(NSError *err) {
            if ([GreatPushErrorUtils isGreatPushError:err]) {
...
/*
 * This is the native side of the GreatPush.getConfiguration method. It isn't
 * currently exposed via the "react-native-great-push" module, and is used
 * internally only by the GreatPush.checkForUpdate method in order to get the
 * app version, as well as the deployment key that was configured in the Info.plist file.
 */
RCT_EXPORT_METHOD(getConfiguration:(RCTPromiseResolveBlock)resolve
                          rejecter:(RCTPromiseRejectBlock)reject)
{
    NSDictionary *configuration = [[GreatPushConfig current] configuration];
    NSError *error;
    if (isRunningBinaryVersion) {
        // isRunningBinaryVersion will not get set to "YES" if running against the packager.
        NSString *binaryHash = [GreatPushUpdateUtils getHashForBinaryContents:[GreatPush binaryBundleURL] error:&error];
        if (error) {
...
/*
 * This method is the native side of the GreatPush.getUpdateMetadata method.
 */
RCT_EXPORT_METHOD(getUpdateMetadata:(GreatPushUpdateState)updateState
                           resolver:(RCTPromiseResolveBlock)resolve
                           rejecter:(RCTPromiseRejectBlock)reject)
{
    NSError *error;
    NSMutableDictionary *package = [[GreatPushPackage getCurrentPackage:&error] mutableCopy];

    if (error) {
        return reject([NSString stringWithFormat: @"%lu", (long)error.code], error.localizedDescription, error);
    } else if (package == nil) {
        // The app hasn't downloaded any GreatPush updates yet,
        // so we simply return nil regardless if the user
        // wanted to retrieve the pending or running update.
        return resolve(nil);
    }

    // We have a GreatPush update, so let's see if it's currently in a pending state.
    BOOL currentUpdateIsPending = [[self class] isPendingUpdate:[package objectForKey:PackageHashKey]];

    if (updateState == GreatPushUpdateStatePending && !currentUpdateIsPending) {
        // The caller wanted a pending update
        // but there isn't currently one.
        resolve(nil);
    } else if (updateState == GreatPushUpdateStateRunning && currentUpdateIsPending) {
        // The caller wants the running update, but the current
        // one is pending, so we need to grab the previous.
        resolve([GreatPushPackage getPreviousPackage:&error]);
    } else {
...
RCT_EXPORT_METHOD(installUpdate:(NSDictionary*)updatePackage
                    installMode:(GreatPushInstallMode)installMode
      minimumBackgroundDuration:(int)minimumBackgroundDuration
                       resolver:(RCTPromiseResolveBlock)resolve
                       rejecter:(RCTPromiseRejectBlock)reject)
{
    NSError *error;
    [GreatPushPackage installPackage:updatePackage
    ...
    if (error) {
        ...
        if (_installMode == GreatPushInstallModeOnNextResume || _installMode == GreatPushInstallModeOnNextSuspend) {
...
/*
 * This method isn't publicly exposed via the "react-native-great-push"
                    ...
                    rejecter:(RCTPromiseRejectBlock)reject)
{
    ...
    BOOL isFirstRun = _isFirstRunAfterUpdate
                        ...
                        && [packageHash isEqualToString:[GreatPushPackage getCurrentPackageHash:&error]];

    resolve(@(isFirstRun));
}

/*
 * This method is the native side of the GreatPush.notifyApplicationReady() method.
 */
...
/*
 * This method is the native side of the GreatPush.restartApp() method.
 */
 ...
 * Note: we don’t recommend to use this method in scenarios other than that (GreatPush will call this method
...
/*
 * This method is the native side of the GreatPush.downloadAndReplaceCurrentBundle()
...
RCT_EXPORT_METHOD(downloadAndReplaceCurrentBundle:(NSString *)remoteBundleUrl)
{
    if ([GreatPush isUsingTestConfiguration]) {
        [GreatPushPackage downloadAndReplaceCurrentBundle:remoteBundleUrl];
    }
}
    ...
    if (needToReportRollback) {
        ...
        if (failedUpdates) {
            ...
            if (lastFailedPackage) {
                resolve([GreatPushTelemetryManager getRollbackReport:lastFailedPackage]);
                return;
            }
        }
    } else if (_isFirstRunAfterUpdate) {
        NSError *error;
        NSDictionary *currentPackage = [GreatPushPackage getCurrentPackage:&error];
        if (!error && currentPackage) {
            resolve([GreatPushTelemetryManager getUpdateReport:currentPackage]);
            return;
        }
    } else if (isRunningBinaryVersion) {
        NSString *appVersion = [[GreatPushConfig current] appVersion];
        resolve([GreatPushTelemetryManager getBinaryUpdateReport:appVersion]);
        return;
    } else {
        NSDictionary *retryStatusReport = [GreatPushTelemetryManager getRetryStatusReport];
...
{
    [GreatPushTelemetryManager recordStatusReported:statusReport];
}

RCT_EXPORT_METHOD(saveStatusReportForRetry:(NSDictionary *)statusReport)
{
    [GreatPushTelemetryManager saveStatusReportForRetry:statusReport];
}
