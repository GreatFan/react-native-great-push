...
@implementation GreatPushConfig {
    NSMutableDictionary *_configDictionary;
}

static GreatPushConfig *_currentConfig;
...
+ (void)initialize
{
    if (self == [GreatPushConfig class]) {
        _currentConfig = [[GreatPushConfig alloc] init];
    }
}
...
- (instancetype)init
{
    ...
    NSString *deploymentKey = [infoDictionary objectForKey:@"GreatPushDeploymentKey"];
    NSString *serverURL = [infoDictionary objectForKey:@"GreatPushServerURL"];
    NSString *publicKey = [infoDictionary objectForKey:@"GreatPushPublicKey"];
    ...
    if (!serverURL) {
        serverURL = @"https://greatpush.azurewebsites.net/";
    }
