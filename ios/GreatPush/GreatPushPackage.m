...
@implementation GreatPushPackage

#pragma mark - Private constants

static NSString *const DiffManifestFileName = @"hotgreatpush.json";
static NSString *const DownloadFileName = @"download.zip";
static NSString *const RelativeBundlePathKey = @"bundlePath";
static NSString *const StatusFile = @"greatpush.json";
...
+ (void)clearUpdates
{
    [[NSFileManager defaultManager] removeItemAtPath:[self getGreatPushPath] error:nil];
}
           ...
           failCallback:(void (^)(NSError *err))failCallback
{
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:newUpdateFolderPath]) {
        ...
        [[NSFileManager defaultManager] removeItemAtPath:newUpdateFolderPath
                                                   error:&error];
    } else if (![[NSFileManager defaultManager] fileExistsAtPath:[self getGreatPushPath]]) {
        [[NSFileManager defaultManager] createDirectoryAtPath:[self getGreatPushPath]
                                  withIntermediateDirectories:YES
                                                   attributes:nil
                                                        error:&error];
                                                        
        // Ensure that none of the GreatPush updates we store on disk are
        // ever included in the end users iTunes and/or iCloud backups
        NSURL *greatPushURL = [NSURL fileURLWithPath:[self getGreatPushPath]];
        [greatPushURL setResourceValue:@YES forKey:NSURLIsExcludedFromBackupKey error:nil];
    }
    ...
    GreatPushDownloadHandler *downloadHandler = [[GreatPushDownloadHandler alloc]
                                                init:downloadFilePath
                                                operationQueue:operationQueue
                                                progressCallback:progressCallback
                                                doneCallback:^(BOOL isZip) {
                                                    NSError *error = nil;
                                                    NSString * unzippedFolderPath = [GreatPushPackage getUnzippedFolderPath];
                                                    ...
                                                    if (isZip) {
                                                        ...
                                                        if (isDiffUpdate) {
                                                            ...
                                                            if (currentPackageFolderPath == nil) {
                                                                ...
                                                                NSString *newUpdateGreatPushPath = [newUpdateFolderPath stringByAppendingPathComponent:[GreatPushUpdateUtils manifestFolderPrefix]];
                                                                [[NSFileManager defaultManager] createDirectoryAtPath:newUpdateGreatPushPath
                                                                                          withIntermediateDirectories:YES
                                                                                                           attributes:nil
                                                                                                                error:&error];
                                                                if (error) {
                                                                ...
                                                                [[NSFileManager defaultManager] copyItemAtPath:[GreatPush bundleAssetsPath]
                                                                                                        toPath:[newUpdateGreatPushPath stringByAppendingPathComponent:[GreatPushUpdateUtils assetsFolderName]]
                                                                                                         error:&error];
                                                                if (error) {
                                                                    failCallback(error);
                                                                    return;
                                                                ...
                                                                [[NSFileManager defaultManager] copyItemAtPath:[[GreatPush binaryBundleURL] path]
                                                                                                        toPath:[newUpdateGreatPushPath stringByAppendingPathComponent:[[GreatPush binaryBundleURL] lastPathComponent]]
                                                                                                         error:&error];
                                                                if (error) {
                                                                    failCallback(error);
                                                                    return;
                                                        ...
                                                        [GreatPushUpdateUtils copyEntriesInFolder:unzippedFolderPath
                                                        ...
                                                        NSString *relativeBundlePath = [GreatPushUpdateUtils findMainBundleInFolder:newUpdateFolderPath
                                                        ...
                                                        if (relativeBundlePath) {
                                                            ...
                                                            NSString *errorMessage = [NSString stringWithFormat:@"Update is invalid - A JS bundle file named \"%@\" could not be found within the downloaded contents. Please ensure that your app is syncing with the correct deployment and that you are releasing your GreatPush updates using the exact same JS bundle file name that was shipped with your app's binary.", expectedBundleFileName];
                                                            
                                                            error = [GreatPushErrorUtils errorWithMessage:errorMessage];
                                                        ...
                                                        NSString *signatureFilePath = [GreatPushUpdateUtils getSignatureFilePath:newUpdateFolderPath];
                                                        BOOL isSignatureAppearedInBundle = [[NSFileManager defaultManager] fileExistsAtPath:signatureFilePath];
                                                        
                                                        if (isSignatureVerificationEnabled) {
                                                            if (isSignatureAppearedInBundle) {
                                                                if (![GreatPushUpdateUtils verifyFolderHash:newUpdateFolderPath
                                                                                              expectedHash:newUpdateHash
                                                                                                     error:&error]) {
                                                                    CPLog(@"The update contents failed the data integrity check.");
                                                                    if (!error) {
                                                                        error = [GreatPushErrorUtils errorWithMessage:@"The update contents failed the data integrity check."];
                                                                    }
                                                                ...
                                                                BOOL isSignatureValid = [GreatPushUpdateUtils verifyUpdateSignatureFor:newUpdateFolderPath
                                                                                                                         expectedHash:newUpdateHash
                                                                ...
                                                                if (!isSignatureValid) {
                                                                    ...
                                                                    if (!error) {
                                                                        error = [GreatPushErrorUtils errorWithMessage:@"The update contents failed code signing check."];
                                                                    }
                                                                    failCallback(error);
                                                                    return;
                                                                } else {
                                                                ...
                                                                error = [GreatPushErrorUtils errorWithMessage:
                                                            ...
                                                            if (isSignatureAppearedInBundle) {
                                                                CPLog(@"Warning! JWT signature exists in greatpush update but code integrity check couldn't be performed" \
...
+ (NSString *)getGreatPushPath
{
    NSString* greatPushPath = [[GreatPush getApplicationSupportDirectory] stringByAppendingPathComponent:@"GreatPush"];
    if ([GreatPush isUsingTestConfiguration]) {
        greatPushPath = [greatPushPath stringByAppendingPathComponent:@"TestPackages"];
    }
...
{
    NSString *packageHash = [GreatPushPackage getCurrentPackageHash:error];
    if (!packageHash) {
        return nil;
    }

    return [GreatPushPackage getPackage:packageHash error:error];
}
...
{
    return [[self getGreatPushPath] stringByAppendingPathComponent:DownloadFileName];
}
...
{
    return [[self getGreatPushPath] stringByAppendingPathComponent:packageHash];
}
...
{
    return [[self getGreatPushPath] stringByAppendingPathComponent:StatusFile];
}

+ (NSString *)getUnzippedFolderPath
{
    return [[self getGreatPushPath] stringByAppendingPathComponent:UnzippedFolderName];
}
