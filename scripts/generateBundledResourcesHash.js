...
var GREAT_PUSH_FOLDER_PREFIX = "GreatPush";
var GREAT_PUSH_HASH_FILE_NAME = "GreatPushHash";
var GREAT_PUSH_HASH_OLD_FILE_NAME = "GreatPushHash.json";
...
function addJsBundleAndMetaToManifest() {
    addFileToManifest(path.dirname(jsBundleFilePath), path.basename(jsBundleFilePath), manifest, function() {
        ...
        addFileToManifest(path.dirname(jsBundleMetaFilePath), path.basename(jsBundleMetaFilePath), manifest, function() {
            ...
            var savedResourcesManifestPath = assetsDir + "/" + GREAT_PUSH_HASH_FILE_NAME;
            fs.writeFileSync(savedResourcesManifestPath, finalHash);

            // "GreatPushHash.json" file name breaks flow type checking.
            // To fix the issue we need to delete "GreatPushHash.json" file and
            // use "GreatPushHash" file name instead to store the hash value.
            // Relates to https://github.com/Microsoft/react-native-great-push/issues/577
            var oldSavedResourcesManifestPath = assetsDir + "/" + GREAT_PUSH_HASH_OLD_FILE_NAME;
...
function addFileToManifest(folder, assetFile, manifest, done) {
    ...
    readStream.pipe(hashStream)
        ...
        .on("finish", function() {
            ...
            manifest.push(path.join(GREAT_PUSH_FOLDER_PREFIX, assetFile).replace(/\\/g, "/") + ":" + fileHash);
            done();
        });
}
