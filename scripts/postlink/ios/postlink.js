...
module.exports = () => {
    ...
    // Fix for https://github.com/Microsoft/react-native-great-push/issues/477
    ...
    https://github.com/microsoft/react-native-great-push#plugin-configuration-ios`);
    }

    var appDelegateContents = fs.readFileSync(appDelegatePath, "utf8");

    // 1. Add the header import statement
    var greatPushHeaderImportStatement = `#import <GreatPush/GreatPush.h>`;
    if (~appDelegateContents.indexOf(greatPushHeaderImportStatement)) {
        console.log(`"GreatPush.h" header already imported.`);
    } else {
    ...
    var newJsCodeLocationAssignmentStatement = "jsCodeLocation = [GreatPush bundleURL];";
    if (~appDelegateContents.indexOf(newJsCodeLocationAssignmentStatement)) {
        console.log(`"jsCodeLocation" already pointing to "[GreatPush bundleURL]".`);
    } else {
    ...
    // 3. Add GreatPushDeploymentKey to plist file
    var parsedInfoPlist = plist.parse(plistContents);
    if (parsedInfoPlist.GreatPushDeploymentKey) {
        console.log(`"GreatPushDeploymentKey" already specified in the plist file.`);
        writePatches();
        ...
        return inquirer.prompt({
            ...
            "message": "What is your GreatPush deployment key for iOS (hit <ENTER> to ignore)"
        }).then(function(answer) {
            parsedInfoPlist.GreatPushDeploymentKey = answer.iosDeploymentKey || "deployment-key-here";
    ...
    function getPlistPath(){
        ...
        //(see https://github.com/Microsoft/react-native-great-push/issues/534#issuecomment-302069326 for details)
