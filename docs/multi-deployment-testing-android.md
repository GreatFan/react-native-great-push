...
> NOTE
>
> Complete demo configured with "multi-deployment testing" feature is [here](https://github.com/Microsoft/react-native-great-push/files/1314118/rncp1004.zip).

The [Android Gradle plugin](http://google.github.io/android-gradle-dsl/current/index.html) allows you to define custom config settings for each "build type" (e.g. debug, release), which in turn are generated as properties on the `BuildConfig` class that you can reference from your Java code. This mechanism allows you to easily configure your debug builds to use your GreatPush staging deployment key and your release builds to use your GreatPush production deployment key.
...
2. Find the `android { buildTypes {} }` section and define `buildConfigField` entries for both your `debug` and `release` build types, which reference your `Staging` and `Production` deployment keys respectively. If you prefer, you can define the key literals in your `gradle.properties` file, and then reference them here. Either way will work, and it's just a matter of personal preference.
    ...
    android {
        ...
        buildTypes {
            debug {
                ...
                // Note: GreatPush updates should not be tested in Debug mode as they are overriden by the RN packager. However, because GreatPush checks for updates in all modes, we must supply a key.
                buildConfigField "String", "GREATPUSH_KEY", '""'
                ...
            }

            releaseStaging {
                ...
                buildConfigField "String", "GREATPUSH_KEY", '"<INSERT_STAGING_KEY>"'
                ...
            }

            release {
                ...
                buildConfigField "String", "GREATPUSH_KEY", '"<INSERT_PRODUCTION_KEY>"'
                ...
            }
        }
        ...
...
4. Pass the deployment key to the `GreatPush` constructor via the build config you just defined, as opposed to a string literal.
...
protected List<ReactPackage> getPackages() {
     return Arrays.<ReactPackage>asList(
         ...
         new GreatPush(BuildConfig.GREATPUSH_KEY, MainApplication.this, BuildConfig.DEBUG), // Add/change this line.
         ...
     );
}
 ```
...
Open up your `MainActivity.java` file and make the following changes:
 ...
 protected List<ReactPackage> getPackages() {
     return Arrays.<ReactPackage>asList(
         ...
         new GreatPush(BuildConfig.GREATPUSH_KEY, this, BuildConfig.DEBUG), // Add/change this line.
         ...
     );
 }
 ```
