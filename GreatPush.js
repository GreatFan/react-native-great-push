import { AcquisitionManager as Sdk } from "great-push/script/acquisition-sdk";
...
let NativeGreatPush = require("react-native").NativeModules.GreatPush;
const PackageMixins = require("./package-mixins")(NativeGreatPush);
...
async function checkForUpdate(deploymentKey = null, handleBinaryVersionMismatchCallback = null) {
   ...
   * different from the GreatPush update they have already installed.
   */
   ...
   *    is available, and GreatPush is making sure they don't get an update that
  ...
  if (!update || update.updateAppVersion ||
    ...
    remotePackage.failedInstall = await NativeGreatPush.isFailedUpdate(remotePackage.packageHash);
...
const getConfiguration = (() => {
  ...
  return async function getConfiguration() {
    if (config) {
      ...
      config = await NativeGreatPush.getConfiguration();
      return config;
    }
  }
})();
...
async function getCurrentPackage() {
  return await getUpdateMetadata(GreatPush.UpdateState.LATEST);
}

async function getUpdateMetadata(updateState) {
  let updateMetadata = await NativeGreatPush.getUpdateMetadata(updateState || GreatPush.UpdateState.RUNNING);
  if (updateMetadata) {
    updateMetadata = {...PackageMixins.local, ...updateMetadata};
    updateMetadata.failedInstall = await NativeGreatPush.isFailedUpdate(updateMetadata.packageHash);
    updateMetadata.isFirstRun = await NativeGreatPush.isFirstRun(updateMetadata.packageHash);
  }
...
async function notifyApplicationReadyInternal() {
  await NativeGreatPush.notifyApplicationReady();
  const statusReport = await NativeGreatPush.getNewStatusReport();
  ...
  try {
    if (statusReport.appVersion) {
      ...
      if (statusReport.status === "DeploymentSucceeded") {
        log(`Reporting GreatPush update success (${label})`);
      } else {
        log(`Reporting GreatPush update rollback (${label})`);
      }
    ...
    NativeGreatPush.recordStatusReported(statusReport);
    resumeListener && AppState.removeEventListener("change", resumeListener);
  } catch (e) {
    log(`Report status failed: ${JSON.stringify(statusReport)}`);
    NativeGreatPush.saveStatusReportForRetry(statusReport);
    // Try again when the app resumes
    if (!resumeListener) {
      resumeListener = async (newState) => {
        if (newState !== "active") return;
        const refreshedStatusReport = await NativeGreatPush.getNewStatusReport();
...
function setUpTestDependencies(testSdk, providedTestConfig, testNativeBridge) {
  ...
  if (testNativeBridge) NativeGreatPush = testNativeBridge;
}

// This function allows only one syncInternal operation to proceed at any given time.
// Parallel calls to sync() while one is ongoing yields GreatPush.SyncStatus.SYNC_IN_PROGRESS.
const sync = (() => {
  ...
  return (options = {}, syncStatusChangeCallback, downloadProgressCallback, handleBinaryVersionMismatchCallback) => {
    ...
    if (syncInProgress) {
      typeof syncStatusCallbackWithTryCatch === "function"
        ? syncStatusCallbackWithTryCatch(GreatPush.SyncStatus.SYNC_IN_PROGRESS)
        : log("Sync already in progress.");
      return Promise.resolve(GreatPush.SyncStatus.SYNC_IN_PROGRESS);
    }
...
async function syncInternal(options = {}, syncStatusChangeCallback, downloadProgressCallback, handleBinaryVersionMismatchCallback) {
  ...
  const syncOptions = {
    ...
    mandatoryInstallMode: GreatPush.InstallMode.IMMEDIATE,
    ...
    : (syncStatus) => {
        switch(syncStatus) {
          ...
          case GreatPush.SyncStatus.UPDATE_INSTALLED:
            if (resolvedInstallMode == GreatPush.InstallMode.ON_NEXT_RESTART) {
              log("Update is installed and will be run on the next app restart.");
            } else if (resolvedInstallMode == GreatPush.InstallMode.ON_NEXT_RESUME) {
  ...
  try {
    await GreatPush.notifyApplicationReady();

    syncStatusChangeCallback(GreatPush.SyncStatus.CHECKING_FOR_UPDATE);
    const remotePackage = await checkForUpdate(syncOptions.deploymentKey, handleBinaryVersionMismatchCallback);

    const doDownloadAndInstall = async () => {
      syncStatusChangeCallback(GreatPush.SyncStatus.DOWNLOADING_PACKAGE);
      const localPackage = await remotePackage.download(downloadProgressCallback);

      // Determine the correct install mode based on whether the update is mandatory or not.
      resolvedInstallMode = localPackage.isMandatory ? syncOptions.mandatoryInstallMode : syncOptions.installMode;

      syncStatusChangeCallback(GreatPush.SyncStatus.INSTALLING_UPDATE);
      await localPackage.install(resolvedInstallMode, syncOptions.minimumBackgroundDuration, () => {
        syncStatusChangeCallback(GreatPush.SyncStatus.UPDATE_INSTALLED);
      });

      return GreatPush.SyncStatus.UPDATE_INSTALLED;
    };
    ...
    if (!remotePackage || updateShouldBeIgnored) {
      ...
      const currentPackage = await GreatPush.getCurrentPackage();
      if (currentPackage && currentPackage.isPending) {
        syncStatusChangeCallback(GreatPush.SyncStatus.UPDATE_INSTALLED);
        return GreatPush.SyncStatus.UPDATE_INSTALLED;
      } else {
        syncStatusChangeCallback(GreatPush.SyncStatus.UP_TO_DATE);
        return GreatPush.SyncStatus.UP_TO_DATE;
      }
    } else if (syncOptions.updateDialog) {
      // updateDialog supports any truthy value (e.g. true, "goo", 12),
      // but we should treat a non-object value as just the default dialog
      if (typeof syncOptions.updateDialog !== "object") {
        syncOptions.updateDialog = GreatPush.DEFAULT_UPDATE_DIALOG;
      } else {
        syncOptions.updateDialog = { ...GreatPush.DEFAULT_UPDATE_DIALOG, ...syncOptions.updateDialog };
      }
      ...
      return await new Promise((resolve, reject) => {
        ...
        if (remotePackage.isMandatory) {
          ...
          dialogButtons.push({
            ...
            onPress: () => {
              syncStatusChangeCallback(GreatPush.SyncStatus.UPDATE_IGNORED);
              resolve(GreatPush.SyncStatus.UPDATE_IGNORED);
            }
          });
        }
        ...
        syncStatusChangeCallback(GreatPush.SyncStatus.AWAITING_USER_ACTION);
    ...
    syncStatusChangeCallback(GreatPush.SyncStatus.UNKNOWN_ERROR);
    log(error.message);
    throw error;
  }
};
...
2. Call the greatPush.sync API in your component instead of using the @greatPush decorator`
    );
  }

  var decorator = (RootComponent) => {
    const extended = class GreatPushComponent extends React.Component {
      componentDidMount() {
        if (options.checkFrequency === GreatPush.CheckFrequency.MANUAL) {
          GreatPush.notifyAppReady();
        } else {
          let rootComponentInstance = this.refs.rootComponent;

          let syncStatusCallback;
          if (rootComponentInstance && rootComponentInstance.greatPushStatusDidChange) {
            syncStatusCallback = rootComponentInstance.greatPushStatusDidChange;
          ...
          if (rootComponentInstance && rootComponentInstance.greatPushDownloadDidProgress) {
            downloadProgressCallback = rootComponentInstance.greatPushDownloadDidProgress;
            if (rootComponentInstance instanceof React.Component) {
          ...
          if (rootComponentInstance && rootComponentInstance.greatPushOnBinaryVersionMismatch) {
            handleBinaryVersionMismatchCallback = rootComponentInstance.greatPushOnBinaryVersionMismatch;
            if (rootComponentInstance instanceof React.Component) {
              handleBinaryVersionMismatchCallback = handleBinaryVersionMismatchCallback.bind(rootComponentInstance);
            }
          }

          GreatPush.sync(options, syncStatusCallback, downloadProgressCallback, handleBinaryVersionMismatchCallback);
          if (options.checkFrequency === GreatPush.CheckFrequency.ON_APP_RESUME) {
            ReactNative.AppState.addEventListener("change", (newState) => {
              newState === "active" && GreatPush.sync(options, syncStatusCallback, downloadProgressCallback);
            });
          }
        }
      }
...
// If the "NativeGreatPush" variable isn't defined, then
// the app didn't properly install the native module,
// and therefore, it doesn't make sense initializing
// the JS interface when it wouldn't work anyways.
if (NativeGreatPush) {
  GreatPush = greatPushify;
  Object.assign(GreatPush, {
    AcquisitionSdk: Sdk,
    checkForUpdate,
    getConfiguration,
    getCurrentPackage,
    ...
    clearUpdates: NativeGreatPush.clearUpdates,
    InstallMode: {
      IMMEDIATE: NativeGreatPush.greatPushInstallModeImmediate, // Restart the app immediately
      ON_NEXT_RESTART: NativeGreatPush.greatPushInstallModeOnNextRestart, // Don't artificially restart the app. Allow the update to be "picked up" on the next app restart
      ON_NEXT_RESUME: NativeGreatPush.greatPushInstallModeOnNextResume, // Restart the app the next time it is resumed from the background
      ON_NEXT_SUSPEND: NativeGreatPush.greatPushInstallModeOnNextSuspend // Restart the app _while_ it is in the background,
    ...
    UpdateState: {
      RUNNING: NativeGreatPush.greatPushUpdateStateRunning,
      PENDING: NativeGreatPush.greatPushUpdateStatePending,
      LATEST: NativeGreatPush.greatPushUpdateStateLatest
    },
    DeploymentStatus: {
  ...
  log("The GreatPush module doesn't appear to be properly installed. Please double-check that everything is setup correctly.");
}

module.exports = GreatPush;
